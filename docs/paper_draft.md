...
- Section 4.5's deep-ensemble numbers do not carry bootstrap confidence intervals, unlike every other result in this paper: the GPU session that produced them persisted only summary metrics, not the raw per-row predictions a bootstrap needs. Computing them requires a future GPU run that saves predictions as well as summaries.
